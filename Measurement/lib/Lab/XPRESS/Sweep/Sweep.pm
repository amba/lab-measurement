...
sub prepaire_config {
	...
	if ( $length_rate > $length_points )
		{
		die "inconsistent definition of sweep_config_data: number of elements in 'rate' larger than number of elements in 'points'.";
		}
	if ( $length_duration > $length_points )
		{
		die "inconsistent definition of sweep_config_data: number of elements in 'duration' larger than number of elements in 'points'.";
		}
	if ( $length_stepwidth > $length_points - 1 and $self->{config}->{mode} ne 'list')
		{
		die "inconsistent definition of sweep_config_data: number of elements in 'stepwidth' larger than number of sweep sequences.";
		}
	...
	if ( $length_interval > $length_points and $self->{config}->{mode} ne 'list')
		{
		die "inconsistent definition of sweep_config_data: number of elements in 'interval' larger than number of sweep sequences.";
		}
	...
	while ( ($length_stepwidth = @{$self->{config}->{stepwidth}}) < $length_points )
		{
		push(@{$self->{config}->{stepwidth}}, @{$self->{config}->{stepwidth}}[-1]);
		}
	...
	while ( ($length_interval = @{$self->{config}->{interval}}) < $length_points )
		{
		push(@{$self->{config}->{interval}}, @{$self->{config}->{interval}}[-1]);
		}
	...
	if ( defined @{$self->{config}->{rate}}[0] and defined @{$self->{config}->{duration}}[0] )
		{
		die 'inconsistent definition of sweep_config_data: rate as well as duration defined. Use only one of both.';
		}
	elsif ( defined @{$self->{config}->{duration}}[0] and @{$self->{config}->{duration}}[0] == 0)
		{
		die 'bad definition of sweep parameters: duration == 0 not allowed';
		}
	elsif ( defined @{$self->{config}->{rate}}[0] and @{$self->{config}->{rate}}[0] == 0)
		{
	...
	if ( defined @{$self->{config}->{stepwidth}}[0] and defined @{$self->{config}->{number_of_points}}[0] )
		{
	...
	if ( $self->{config}->{mode} eq 'continuous' )
		{	
		if ( not defined @{$self->{config}->{rate}}[0] or not defined @{$self->{config}->{duration}}[0] )
			{
		...
		if ( not defined @{$self->{config}->{stepwidth}}[0] or not defined @{$self->{config}->{number_of_points}}[0] )
			{
		...
		if ( not defined @{$self->{config}->{rate}}[0] )
			{
			die "inconsistent definition of sweep_config_data: 'rate' needs to be defined in sweep mode 'list'";
			}
		}
		...
		while(1)
			...
			if ( defined @{$self->{slaves}}[0] )
				...
				foreach my $slave (@{$self->{slaves}})
					...
					my $extensions = dclone(\@{$self->{filename_extensions}});
	...
	if ( not defined $self->{master} )
		...
		foreach my $slave (@{$self->{slaves}} )
			...
			if ( $slave->{config}->{mode} =~ /step/ )
				{
				$text .=  "\t\t|--> stepwidth = @{$slave->{config}->{stepwidth}}\n";
				}
			$text .=  "\t\t|--> rate = @{$slave->{config_original}->{rate}}\n";
			$text .=  "\t\t|--> duration = @{$slave->{config_original}->{duration}}\n";
